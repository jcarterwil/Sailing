...
2. Open every PR against `main` as **Ready for review**, not as a draft. Create a draft only when the user explicitly requests one. Immediately after opening the ready PR: (a) post `@codex review` in the PR conversation, and (b) **always run Bugbot** on the branch changes (Cursor Bugbot / `bugbot` subagent). Do both on every agent-opened PR; do not skip Bugbot because CI or Codex already ran. GitHub Copilot and Claude (`.github/workflows/claude-review.yml`) also run automatic advisory reviews on ready PRs; they do not run on drafts or automatically re-review every push. Address or explain material findings and resolve their review conversations. Reviewer errors or exhausted quotas do not block merging.
