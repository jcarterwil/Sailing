...
- **Start line / race clock:** `LoadedTrack.extras` carries VKX timerEvents/linePings (null for CSV). Derive `startsMs` + `startLine` once in `race-replay.tsx` via `fleetStarts` / `startLineAt` (`src/lib/analytics/start-line.ts`); pass into MapView / PlaybackControls / Timeline. Never fabricate a line from one end.
