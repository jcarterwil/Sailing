...
`playback-store.ts` (zustand) holds `timeMs`, `playing`, `speed`, `trailMode`, and `rangeSel` (the brush selection). Keep it minimal; derived values are computed by consumers from the track arrays + `track-index.ts`, not stored.
