...
- **Supabase project:** ref `mmyogyxvgwfmrqjcsguz` (name `sailing`), region `us-east-1`, Postgres 17. Created via the **Vercel–Supabase integration**, so it lives under a Vercel-managed org (`vercel_icfg_…`).
 ...
 - **Migrations auto-apply on merge to `main`** via `.github/workflows/supabase-migrate.yml` (path filter on `supabase/migrations/**`). Requires GitHub Actions secrets `SUPABASE_ACCESS_TOKEN` and `SUPABASE_DB_PASSWORD`.
