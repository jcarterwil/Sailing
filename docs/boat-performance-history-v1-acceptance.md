...
| Race-only metrics unavailable with reasons on Practices | **PARTIAL** | Observation contract (#172 / #180) + query layer (#173 / #181) on `main` with Practice exclusion reasons. UI Practice cards still need #174. Defect: #183. |
| Queries bounded; no raw tracks returned | **PASS (API)** | `#173` / `#181` on `main`: `GET /api/boats/[boatId]/performance-history` is `can_view_boat`-gated, ≤250, compact rows only. Acceptance suite asserts when route present. |
